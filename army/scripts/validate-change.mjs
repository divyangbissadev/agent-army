...
// Usage: node .claude/army/scripts/validate-change.mjs <change-folder-path>
...
if (proposal) {
  for (const section of ["why", "what changes", "what does not change"]) {
    if (!proposal.toLowerCase().includes(section))
      findings.push(`proposal.md missing section: ${section}`);
  }
}
...
if (spec) {
  ...
  const requirements = (spec.match(/SHALL/g) || []).length;
  const scenarios = (spec.match(/^#{0,4}\s*Scenario:/gm) || []).length;
