...
1. **Problem.** Who hurts, when, how badly. One paragraph. If you cannot name
...
3. **Smallest valuable slice.** The MVP that a real user would thank you for,
   cut ruthlessly. Then the next two slices, so engineering sees the direction
...
6. **Risks and open questions.** Ranked, each with the cheapest experiment
